...
export function IotView() {
  ...
  return (
    <div className="space-y-4">
      ...
      {/* ── Header banner ─────────────────────────────────────────────────── */}
      <div className="rounded-lg border border-[#8b9dff]/40 bg-[#22254a] p-3 flex flex-wrap items-center gap-4">
        <div className="flex items-center gap-2">
          <Terminal className="h-4 w-4 text-[#8b9dff]" />
          <span className="font-mono text-sm font-bold text-[#8b9dff]">DEVICE-001</span>
          <span className="text-[10px] text-[#54697c]">IoT CA · TLS mutuo · TTL 72h</span>
        </div>
        <div className="flex items-center gap-4 ml-auto text-xs font-mono">
          <span className="flex items-center gap-1 text-[#79cf9c]">
          ...
          <span className="flex items-center gap-1 text-[#8b9dff]">
      ...
      <div
        ref={logRef}
        className="rounded-lg border border-[#22384d] bg-[#070d12] p-4 h-[500px] overflow-y-auto font-mono text-xs leading-relaxed scroll-smooth"
        style={{ fontFamily: "'IBM Plex Mono', 'Courier New', monospace" }}
      >
        {lines.map((l) => (
          <div key={l.id} className="flex gap-3 hover:bg-[#0f1e2c]/50 rounded px-1 py-0.5">
            <span className="shrink-0 text-[#3a5060] tabular-nums w-16">{l.time}</span>
        ...
        {simRunning && (
          <div className="flex gap-3 px-1 py-0.5">
            <span className="shrink-0 text-[#3a5060] w-16">{fmtSimLabel(simTimeHours)}</span>
            <span className="shrink-0 w-7 text-[#4fb8c4]">SYS</span>
            <span className="text-[#4fb8c4]">
              <span className="animate-pulse">▌</span>
            </span>
          </div>
        )}
        {!simRunning && simTimeHours === 0 && (
          <div className="mt-4 text-[#3a5060]">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          { label: "Temp. interna",  value: `${caseData.tempInternal.toFixed(2)} °C`, color: caseData.tempInternal > 4.5 ? "#cfa25e" : "#79cf9c" },
          { label: "Batería",        value: `${caseData.battery}%`,                  color: caseData.battery < 30 ? "#e5626a" : "#79cf9c" },
          { label: "Conectividad",   value: caseData.connectivity,                   color: "#8b9dff" },
          { label: "Firma ECDSA",    value: "OK · IoT-CA",                            color: "#8b9dff" },
        ].map((m) => (
          <div key={m.label} className="rounded-lg border border-[#22384d] bg-[#0f1e2c] p-3">
            <p className="text-[10px] uppercase tracking-wide text-[#54697c]">{m.label}</p>
            <p className="mt-1 font-mono text-sm font-bold" style={{ color: m.color }}>{m.value}</p>
          </div>
        ))}
      </div>

      {/* ── Nota: sin datos clínicos ──────────────────────────────────────── */}
      <div className="rounded-lg border border-dashed border-[#22384d] p-3 text-center">
        <p className="text-xs text-[#54697c]">
